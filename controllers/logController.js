...
const getTimingSummary = asyncHandler(async (req, res) => {
    ...
    const [summary] = await Log.aggregate([
        { $match: match },
        {
            $group: {
                ...
                face_detection_min: { $min: "$timing.face_detection_ms" },
                face_detection_max: { $max: "$timing.face_detection_ms" },
                face_detection_mean: { $avg: "$timing.face_detection_ms" },
                face_detection_std_dev: { $stdDevPop: "$timing.face_detection_ms" },
                feature_extraction_min: { $min: "$timing.feature_extraction_ms" },
                feature_extraction_max: { $max: "$timing.feature_extraction_ms" },
                feature_extraction_mean: { $avg: "$timing.feature_extraction_ms" },
                feature_extraction_std_dev: { $stdDevPop: "$timing.feature_extraction_ms" },
                embedding_generation_min: { $min: "$timing.embedding_generation_ms" },
                embedding_generation_max: { $max: "$timing.embedding_generation_ms" },
                embedding_generation_mean: { $avg: "$timing.embedding_generation_ms" },
                embedding_generation_std_dev: { $stdDevPop: "$timing.embedding_generation_ms" },
    ...
    return res.status(200).json({
        ...
        stages: [
            ...
            row("template_decryption", "Stored Embedding Decryption"),
            row("match_computation", "Match Score Computation"),
